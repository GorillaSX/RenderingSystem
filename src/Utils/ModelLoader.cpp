...
ModelLoaderResult ModelLoader::load(const ModelLoaderInfo& info)
{
	...
	while (getLine(fileBufferPtr, fileBufferLength, lineStartIndex, lineEndIndex))
	{
		...
		if (!info.loadOnlyMaterials)
		{
			...
			else if (compareWord(fileBufferPtr, wordStartIndex, wordEndIndex, "usemtl")) // select material
			{
				...
				{
					std::runtime_error(tfm::format("Could not find material named \"%s\"", materialName));
					currentMaterialId = info.defaultMaterialId;
				}
			}
		}
	...
	printf("OBJ file reading finished (time: %s, vertices: %lu, normals: %lu, texcoords: %lu, triangles: %lu, materials: %lu, textures: %lu)", timer.getElapsed().getString(true).c_str(), vertices.size(), normals.size(), texcoords.size(), result.triangles.size(), result.materials.size(), result.textures.size());
...
void ModelLoader::processMaterialFile(const std::string& rootDirectory, const std::string& mtlFileName, ModelLoaderResult& result)
{
	...
	printf("Reading MTL file (%s)", absoluteMtlFileName.c_str());
	...
	while (std::getline(file, line))
	{
		...
		if (part == "newmtl") // new material
		{
			...
			currentMaterialId = result.materials.size();
		...
		{
			ss >> currentMaterial.ambient.r;
			ss >> currentMaterial.ambient.g;
			ss >> currentMaterial.ambient.b;
		}
		else if(part == "Kd")
		{
			ss >> currentMaterial.diffuse.r;
			ss >> currentMaterial.diffuse.g;
			ss >> currentMaterial.diffuse.b;
		}
		else if(part == "Ks")
		{
			ss >> currentMaterial.specular.r;
			ss >> currentMaterial.specular.g;
			ss >> currentMaterial.specular.b;
		}
		else if(part == "Kt")
		{
			ss >> currentMaterial.transmittance.r;
			ss >> currentMaterial.transmittance.g;
			ss >> currentMaterial.transmittance.b;
		}
		else if(part == "Ni")
		{
			ss >> currentMaterial.ior;
		}
		else if(part == "Ns")
		{
		...
		else if(part == "map_Ka")
		{
			...
			texture.id = result.textures.size();
			currentMaterial.ambientTextureId = texture.id;
		...
		else if(part == "map_Kd")
		{
			...
			texture.type = TextureType::IMAGE;
			texture.id = result.textures.size();
			currentMaterial.diffuseTextureId = texture.id;
		...
		else if(part == "map_Ks")
		{
			...
			texture.type = TextureType::IMAGE;
			texture.id = result.textures.size();
			currentMaterial.specularTextureId = texture.id;
		...
		else if(part == "map_Ns")
		{
			...
			texture.type = TextureType::IMAGE;
			texture.id = result.textures.size();
			currentMaterial.shininessTextureId = texture.id;
		...
		else if(part == "map_bump" || part == "bump")
		{
			Texture texture;
			texture.type = TextureType::IMAGE;
			texture.id = result.textures.size();
			currentMaterial.bumpTextureId = texture.id;
...
bool ModelLoader::processFace(const char* buffer, uint32_t lineStartIndex, uint32_t lineEndIndex, uint32_t lineNumber, ModelLoaderResult& result)
{
	...
	for (uint32_t i = 0; i < 4; ++i)
	{
		...
		{
			printf("Vertex index (%d) was out of bounds (line: %d)", vertexIndex, lineNumber);
			return false;
		}
		...
		if (hasTexcoords)
		{
			...
			{
				printf("Texcoord index (%d) was out of bounds (line: %d)", texcoordIndex, lineNumber);
				return false;
			}
		...
		if (hasNormals)
		{
			...
			{
				printf("Normal index (%d) was out of bounds (line: %d)", normalIndex, lineNumber);
				return false;
			}
	...
	if (vertexCount < 3)
	{
		printf("Too few vertices (%d) in a face (line: %d)", vertexCount, lineNumber);
		return false;
	}
